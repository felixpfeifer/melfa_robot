...
void MelfaHW::write_first(void) {
    ...
    if (size != sizeof(send_buff_)) {
        ROS_ERROR ("Cannot send packet to MELFA controller. Check the configuration.");
        exit(1);
    }
}
...
uint8_t MelfaHW::read_IO_sending_type(void) {
    return sendingType;
}

void MelfaHW::write_IO_sending_Type(uint8_t type) {
    sendingType = type;
}
void MelfaHW::write_IO_reading_Type(uint8_t type) {
    readingType = type;
}
